...
def get_posts():
    ...
    offset = page * 20
    posts = db.session.query(Post, Account).join(Account, Post.poster_id == Account.id).order_by(Post.created_at.desc()).limit(20).offset(offset).all()
    post_list = [{"id": post.id, "url": post.url, "title": post.title, "poster_name": account.name, "created_at": post.created_at} for post, account in posts]
